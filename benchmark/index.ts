...
function str(length: number) {
  return Array.from({ length: length }, (v, a) => 'x').join('');
}

fs.writeFileSync(path.join(__dirname, 'input.txt'), 
[Array.from({ length: 5000 }, (v, a) => str(Math.ceil(Math.random() * 5000))).join('\n')]
)
